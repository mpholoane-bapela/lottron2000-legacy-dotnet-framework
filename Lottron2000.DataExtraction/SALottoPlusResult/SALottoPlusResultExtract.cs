...
{
    public static class SALottoPlusResultExtract
    {
    // Add non default field values for ItemID, entityID and CheckSum
        public static void AddFieldValues()
        {
            var allItems = SALottoPlusResultBL.GetAll().OrderBy(a => a.DrawNo);

            int index = 1;
            foreach (var item in allItems)
            {
                item.ItemID = index;
                item.SALottoPlusResultID = Guid.NewGuid().ToString();
                item.CheckSum = item.Ball1 + item.Ball2 + item.Ball3 + item.Ball4 + item.Ball5 + item.Ball6;

                SALottoPlusResultBL.Update(item);
                index++;
            }
        }
